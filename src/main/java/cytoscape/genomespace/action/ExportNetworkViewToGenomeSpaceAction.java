 ...
 * method.
 */
public class ExportNetworkViewToGenomeSpaceAction extends AbstractCyAction {
	private static final long serialVersionUID = 9988760123456789L;
	private static final Logger logger = LoggerFactory.getLogger(ExportNetworkViewToGenomeSpaceAction.class);
	...
	public ExportNetworkViewToGenomeSpaceAction(CyApplicationManager cyApplicationManager, CyNetworkViewManager cyNetworkViewManager,  DialogTaskManager dialogTaskManager, CyNetworkViewWriterManager cyNetworkViewWriterManager, GenomeSpaceContext gsContext, JFrame frame, ImageIcon icon) {
		// Give your action a name here
		super("Network View to GenomeSpace...", cyApplicationManager, "networkAndView", cyNetworkViewManager);

		// Set the menu you'd like here.  Plugins don't need
		// to live in the Plugins menu, so choose whatever
		// is appropriate!
		setPreferredMenu("File.Export");
		setMenuGravity(1.12f);
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			String xgmmlFilterDescription = null;
			for (CyFileFilter filter: filters) {
				filterMap.put(filter.getDescription(), filter);
				if(filter.getExtensions().contains("xgmml"))
					xgmmlFilterDescription = filter.getDescription();
			}
